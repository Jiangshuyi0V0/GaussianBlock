...
class Metrics:
    ...
    def update(self, *name_val, N=1, new_names=[1]):
        # count = len(names)
        if len(name_val) == 1:
            count = len(new_names)
            if (count > len(self.names) - 1 and self.names[0] == 'time/img') or (count > len(self.names) and self.names[0] != 'time/img'):
                self.names = [self.names[0]] + new_names if self.names[0] == 'time/img' else new_names
                with open(self.log_file, mode='w') as f:
                    f.write("iteration\tepoch\tbatch\t" + "\t".join(self.names) + "\n")
            d = name_val[0]
