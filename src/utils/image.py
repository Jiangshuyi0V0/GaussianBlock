...
class ImageLogger:
    ...
    def save(self, images, it=None, **Exp_cfg):
        sep_Blocks = Exp_cfg.get('separate_blocks', False)
        assert images.dim() == 5 if sep_Blocks else 4
        if not self.log_data:
            return None
        ...
        if not hasattr(self, '_img_size'):
            self._img_size = tuple(images.shape[2:]) if not sep_Blocks else tuple(images.shape[3:])
        for k in range(self.n_images):
            if it is not None:
                if not sep_Blocks:
                    convert_to_img(images[k]).save(self.log_dir / f'img{k}' / 'evolution' / f'{it}.{self.out_ext}')
                else:
                    for blk_idx, blk_img in enumerate(images[k]):
                        convert_to_img(blk_img).save(self.log_dir / f'img{k}' / 'evolution' / f'{it}_blk{blk_idx}.{self.out_ext}')
