...
class Renderer(nn.Module):
    def __init__(self, img_size, **kwargs):
        ...
        raster_settings = RasterizationSettings(image_size=(self.img_size[0] * 4, self.img_size[1] * 4),
                                                **self.r_kwargs)
    ...
    def render_edges(self, meshes, R, T, image_size=None, linewidth=1, return_pix2face=False, faces_per_pixel=1,
                     render_vert=False, return_visible_face=False):
        ...
        visible_face_indices = torch.unique(fragments.pix_to_face[fragments.pix_to_face >= 0])
        if return_visible_face:
    ...
    def draw_edges(self, img, meshes, R=None, T=None, colors=None, linewidth=1, antialias=True, render_vert=False):
        if R is None:
        ...
        if not render_vert:
            mask, pix2face = self.render_edges(meshes, R, T, image_size=img_size, linewidth=linewidth,
                                               return_pix2face=True,
                                               render_vert=render_vert)
        else:
            mask, pix2face, verts_2d, vertex_images = self.render_edges(meshes, R, T, image_size=img_size,
                                                                        linewidth=linewidth,
                                                                        return_pix2face=True,
                                                                        render_vert=render_vert)
        ...
        if antialias:
            if render_vert:
                point_mask = torch.zeros((bsz, 1, height, width)).to(img.device)
                point_size = 5
                for i in range(bsz):
                    x_coords = verts_2d[i, :, 0].clamp(0, width - 1).long()
                    y_coords = verts_2d[i, :, 1].clamp(0, height - 1).long()

                    # Create a grid for each point
                    for dx in range(-point_size, point_size + 1):
                        for dy in range(-point_size, point_size + 1):
                            new_x_coords = torch.clamp(x_coords + dx, min=0, max=width - 1)
                            new_y_coords = torch.clamp(y_coords + dy, min=0, max=height - 1)
                            point_mask[i, 0, new_y_coords, new_x_coords] = 1
                mask, face_img, point_mask = [F.avg_pool2d(t, kernel_size=4, stride=4) for t in
                                              [mask, face_img, point_mask]]
        ...
        if not render_vert:
            ...
            color_image = torch.zeros(bsz, 3, height, width, dtype=torch.uint8).to(img.device)
            color_image[:, 0, :, :] = 1  # Red
            img = img * (1 - point_mask) + point_mask * color_image
            return img

    def draw_verts(self, img, mesh, R=None, T=None, colors=None, bsz=4, dry_draw=False, scene=1, draw_verts=None):
        scene_N = scene
        if R is None:
            R = torch.eye(3, device=mesh.device)[None].expand(len(mesh), -1, -1)
        if T is None:
            T = torch.zeros(1, 3, device=mesh.device).expand(len(mesh), -1)
        if colors is None:
            colors = (1, 0, 0)
        img_size = img.shape[-2:] if isinstance(img, torch.Tensor) else img.size[::-1]
        width = img_size[1]
        ...
        color_image = torch.zeros(bsz, 3, img_size[0], img_size[1], dtype=torch.uint8).to(img.device)
        color_image[:, 0, :, :] = 1  # Red

        meshes = mesh.extend(bsz)
        visible_face_indices = self.render_edges(meshes, R, T, image_size=img_size, return_visible_face=True)
        visible_vert_indices = torch.unique(meshes.faces_packed()[visible_face_indices].flatten())
        thresholds = []

        for i in range(bsz):
            thresholds.append((i * 42 * scene_N, i * 42 * scene_N + 42 * scene_N - 1))
        visible_vert_indices = [visible_vert_indices[(visible_vert_indices >= low) & (visible_vert_indices <= high)] for low, high in thresholds]

        # Transform mesh vertices to screen space
        verts = mesh.verts_packed()  # Get vertices
        verts_ndc = self.cameras.transform_points_screen(verts, image_size=img_size)
        # Basic visibility based on NDC z-values and image bound (not occlusion-aware)
        verts_2d = verts_ndc[..., :2] # Take only visible vertices based on z-value
        if len(verts_2d.shape) == 2:
            verts_2d = verts_2d.unsqueeze(0)

        vis_verts = []
        vis_idx = []
        for i, idx in enumerate(visible_vert_indices):
            bidx = idx - i * 42 * scene_N
            visible = (0 <= verts_2d[i][bidx][:, 0]) & (verts_2d[i][bidx][:, 0] <= width) & (0 <= verts_2d[i][bidx][:, 1]) & (verts_2d[i][bidx][:, 1] <= height)# & (verts_ndc[..., 2] >= -1) & (verts_ndc[..., 2] <= 1)
            vis_verts.append(verts_2d[i][bidx][visible])
            vis_idx.append(idx[visible])
        if dry_draw:
            return vis_verts, vis_idx

        else:
            vis_verts = draw_verts
            point_size = 2
            point_mask = torch.zeros((bsz, 1, height, width)).to(img.device)
            if vis_verts is None:
                return img, [[]]
            for b_idx, idx in enumerate(vis_verts):
                x_coords = idx[:, 0].clamp(0, width - 1).long()
                y_coords = idx[:, 1].clamp(0, height - 1).long()
                for dx in range(-point_size, point_size + 1):
                    for dy in range(-point_size, point_size + 1):
                        new_x_coords = torch.clamp(x_coords + dx, min=0, max=width - 1)
                        new_y_coords = torch.clamp(y_coords + dy, min=0, max=height - 1)
                        point_mask[b_idx, 0, new_y_coords, new_x_coords] = 1

            img = img * (1 - point_mask) + point_mask * color_image
            return img, vis_verts
