...
def create_train_val_test_loader(cfg, rank=None, world_size=None):
    kwargs = {**cfg["dataset"], **cfg['Exp_setting']['Exp_setting']}
...
def create_single_loader(cfg, rank=None, world_size=None):
    kwargs = {**cfg["dataset"]}
    name = kwargs.pop("name")
    train = get_dataset(name)(split="train", **kwargs)
    bs, nw = cfg["training"]["colored_batch_size"], cfg["training"].get("colored_n_workers", 4)

    single_loader = DataLoader(train, batch_size=bs, num_workers=nw, pin_memory=True)
    print_log(f"colored_Dataset '{name}' init: kwargs={kwargs}, n_train={len(train)}, bs={bs}, n_work={nw}")
